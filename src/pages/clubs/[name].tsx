import React from "react";
import {Layout} from "../../components/common/Layout";
import PlaceholderProfile from "../../assets/vectors/clubs/PlaceholderProfile";
import Footer from "../../components/common/Footer";
import PlaceholderSuggestion from "../../assets/vectors/clubs/PlaceholderSuggestion";

const Page = () => {

  return (
    <Layout>
      <div className="flex justify-center">
        <div className="max-w-3xl">
        <div className="flex flex-col justify-center items-center mt-20">
          <h1 className="text-6xl font-bold text-blue-400">ชมรมซัดหน้าพี่มาร์ค</h1>
          <h1 className="text-blue-400 font-medium text-2xl mt-2">ชมรม | 999 คน</h1>
        </div>
        <div className="flex flex-col justify-center items-center mt-12">
          <div className="bg-gray-300 h-96 w-full">
          </div>
          <h1 className="text-gray-300 font-semibold text-xl pt-3">ภาพบรรยากาศภายในชมรมซัดหน้าพี่ม้าก</h1>
        </div>
        <div className="mt-20">
          <h1 className="font-medium text-3xl">ชมรมนี้ทำอะไร</h1>
          <p className="text-gray-500 text-xl mt-12">
            มาบอกสูตรเด็ดของอาหารคลีนกันอีกแล้วจ้าอาหารคลีนไม่มีวันเอาท์แน่นอนสำหรับสาวๆที่รักสุขภาพต้องการห่างไกลจากเมนูที่ทำร้ายร่างกายแต่อะไรที่ไม่ดีต่อสุขภาพมักจะอร่อยอยู่เสมอๆอย่างเมนูผัดกะเพราที่ต้องเจอทุกมื้อกลางวันแต่สาวกอาหารคลีนไม่ต้องห่วงว่าจะอดกินกะเพราวันนี้มาแนะนำเมนูกะเพราไก่ไข่ดาวแบบฉบับอาหารคลีนเตรียมตัวเข้าครัวกันได้เลย
          </p>
          <div className="flex flex-col justify-center items-center mt-12">
            <div className="bg-gray-300 h-96 w-full">
            </div>
            <h1 className="text-gray-300 font-semibold text-xl pt-3">ภาพบรรยากาศภายในชมรมซัดหน้าพี่ม้าก</h1>
          </div>
        </div>
        <div className="mt-20">
          <h1 className="font-medium text-3xl">ได้อะไรจากชมรม</h1>
          <p className="text-gray-500 text-xl mt-12">
            มาบอกสูตรเด็ดของอาหารคลีนกันอีกแล้วจ้าอาหารคลีนไม่มีวันเอาท์แน่นอนสำหรับสาวๆที่รักสุขภาพต้องการห่างไกลจากเมนูที่ทำร้ายร่างกายแต่อะไรที่ไม่ดีต่อสุขภาพมักจะอร่อยอยู่เสมอๆอย่างเมนูผัดกะเพราที่ต้องเจอทุกมื้อกลางวันแต่สาวกอาหารคลีนไม่ต้องห่วงว่าจะอดกินกะเพราวันนี้มาแนะนำเมนูกะเพราไก่ไข่ดาวแบบฉบับอาหารคลีนเตรียมตัวเข้าครัวกันได้เลย
          </p>
          <div className="flex flex-col justify-center items-center mt-12">
            <div className="bg-gray-300 h-96 w-full">
            </div>
            <h1 className="text-gray-300 font-semibold text-xl pt-3">ภาพบรรยากาศภายในชมรมซัดหน้าพี่ม้าก</h1>
          </div>
        </div>
        <div className="mt-20">
          <h1 className="font-medium text-3xl">ผลงานของชมรม</h1>
          <p className="text-gray-500 text-xl mt-12">
            มาบอกสูตรเด็ดของอาหารคลีนกันอีกแล้วจ้าอาหารคลีนไม่มีวันเอาท์แน่นอนสำหรับสาวๆที่รักสุขภาพต้องการห่างไกลจากเมนูที่ทำร้ายร่างกายแต่อะไรที่ไม่ดีต่อสุขภาพมักจะอร่อยอยู่เสมอๆอย่างเมนูผัดกะเพราที่ต้องเจอทุกมื้อกลางวันแต่สาวกอาหารคลีนไม่ต้องห่วงว่าจะอดกินกะเพราวันนี้มาแนะนำเมนูกะเพราไก่ไข่ดาวแบบฉบับอาหารคลีนเตรียมตัวเข้าครัวกันได้เลย
          </p>
        </div>
        <div className="mt-20">
          <h1 className="font-medium text-3xl">รีวิวจากรุ่นพี่</h1>
          <div className="flex flex-row mt-14">
            <div className="flex flex-col">
              <div className="flex">
                <PlaceholderProfile />
              </div>
              <div className="flex flex-col text-gray-500">
                <h1 className="text-2xl font-black">ตะวัน</h1>
                <span className="text-xs">เตรียมอุดม 83</span>
                <span className="w-max text-xs">IG: tucmc_official</span>
              </div>
            </div>
            <div className="flex flex-col ml-4">
              <div className="relative">
                <span className="absolute text-gray-300 text-8xl left-10 top-6">“</span>
              </div>
              <div className="bg-whtie text-xl shadow-lg text-gray-500 rounded-3xl py-16 px-16">
                มาบอกสูตรเด็ดของอาหารคลีนกันอีกแล้วจ้าาอาหารคลีนไม่มีวันเอาท์แน่นอนสำหรับสาวๆที่รักสุขภาพต้องการห่างไกลจากเมนูที่ทำร้ายร่างกายแต่อะไรที่ไม่ดีต่อสุขภาพมักจะอร่อยอยู่เสมอๆอย่างเมนูผัดกะเพราที่ต้องเจอทุกมื้อกลางวัน
                มาบอกสูตรเด็ดของอาหารคลีนกันอีกแล้วจ้าาอาหารคลีนไม่มีวันเอาท์แน่นอนสำหรับสาวๆที่รักสุขภาพต้องการห่างไกลจากเมนูที่ทำร้ายร่างกายแต่อะไรที่ไม่ดีต่อสุขภาพมักจะอร่อยอยู่เสมอๆอย่างเมนูผัดกะเพราที่ต้องเจอทุกมื้อกลางวัน
              </div>
              <div className="relative">
                <span className="absolute text-gray-300 text-8xl right-16 -top-20">“</span>
              </div>
            </div>
          </div>
          <div className="flex flex-row mt-14">
            <div className="flex flex-col">
              <div className="flex">
                <PlaceholderProfile />
              </div>
              <div className="flex flex-col text-gray-500">
                <h1 className="text-2xl font-black">ตะวัน</h1>
                <span className="text-xs">เตรียมอุดม 83</span>
                <span className="w-max text-xs">IG: tucmc_official</span>
              </div>
            </div>
            <div className="flex flex-col ml-4">
              <div className="relative">
                <span className="absolute text-gray-300 text-8xl left-10 top-6">“</span>
              </div>
              <div className="bg-whtie text-xl shadow-lg text-gray-500 rounded-3xl py-16 px-16">
                มาบอกสูตรเด็ดของอาหารคลีนกันอีกแล้วจ้าาอาหารคลีนไม่มีวันเอาท์แน่นอนสำหรับสาวๆที่รักสุขภาพต้องการห่างไกลจากเมนูที่ทำร้ายร่างกายแต่อะไรที่ไม่ดีต่อสุขภาพมักจะอร่อยอยู่เสมอๆอย่างเมนูผัดกะเพราที่ต้องเจอทุกมื้อกลางวัน
                มาบอกสูตรเด็ดของอาหารคลีนกันอีกแล้วจ้าาอาหารคลีนไม่มีวันเอาท์แน่นอนสำหรับสาวๆที่รักสุขภาพต้องการห่างไกลจากเมนูที่ทำร้ายร่างกายแต่อะไรที่ไม่ดีต่อสุขภาพมักจะอร่อยอยู่เสมอๆอย่างเมนูผัดกะเพราที่ต้องเจอทุกมื้อกลางวัน
              </div>
              <div className="relative">
                <span className="absolute text-gray-300 text-8xl right-16 -top-20">“</span>
              </div>
            </div>
          </div>
          <div className="flex flex-row mt-14">
            <div className="flex flex-col">
              <div className="flex">
                <PlaceholderProfile />
              </div>
              <div className="flex flex-col text-gray-500">
                <h1 className="text-2xl font-black">ตะวัน</h1>
                <span className="text-xs">เตรียมอุดม 83</span>
                <span className="w-max text-xs">IG: tucmc_official</span>
              </div>
            </div>
            <div className="flex flex-col ml-4">
              <div className="relative">
                <span className="absolute text-gray-300 text-8xl left-10 top-6">“</span>
              </div>
              <div className="bg-whtie text-xl shadow-lg text-gray-500 rounded-3xl py-16 px-16">
                มาบอกสูตรเด็ดของอาหารคลีนกันอีกแล้วจ้าาอาหารคลีนไม่มีวันเอาท์แน่นอนสำหรับสาวๆที่รักสุขภาพต้องการห่างไกลจากเมนูที่ทำร้ายร่างกายแต่อะไรที่ไม่ดีต่อสุขภาพมักจะอร่อยอยู่เสมอๆอย่างเมนูผัดกะเพราที่ต้องเจอทุกมื้อกลางวัน
                มาบอกสูตรเด็ดของอาหารคลีนกันอีกแล้วจ้าาอาหารคลีนไม่มีวันเอาท์แน่นอนสำหรับสาวๆที่รักสุขภาพต้องการห่างไกลจากเมนูที่ทำร้ายร่างกายแต่อะไรที่ไม่ดีต่อสุขภาพมักจะอร่อยอยู่เสมอๆอย่างเมนูผัดกะเพราที่ต้องเจอทุกมื้อกลางวัน
              </div>
              <div className="relative">
                <span className="absolute text-gray-300 text-8xl right-16 -top-20">“</span>
              </div>
            </div>
          </div>
        </div>
        <div className="mt-20">
          <h1 className="text-2xl font-bold text-center mb-6">ชมรมอื่นๆ</h1>
          <div className="flex flex-wrap justify-center space-x-4">
            <div className="text-center mb-10">
              <PlaceholderSuggestion className="pb-2 w-60"/>
              <span className="font-semibold">พัฒนาศักยภาพทางวิทยาศาสตร์</span>
            </div>
            <div className="text-center mb-10">
              <PlaceholderSuggestion className="pb-2 w-60"/>
              <span className="font-semibold">พัฒนาศักยภาพทางวิทยาศาสตร์</span>
            </div>
            <div className="text-center mb-10">
              <PlaceholderSuggestion className="pb-2 w-60"/>
              <span className="font-semibold">พัฒนาศักยภาพทางวิทยาศาสตร์</span>
            </div>
            <div className="text-center mb-10">
              <PlaceholderSuggestion className="pb-2 w-60"/>
              <span className="font-semibold">พัฒนาศักยภาพทางวิทยาศาสตร์</span>
            </div>
            <div className="text-center mb-10">
              <PlaceholderSuggestion className="pb-2 w-60"/>
              <span className="font-semibold">พัฒนาศักยภาพทางวิทยาศาสตร์</span>
            </div>
            <div className="text-center mb-10">
              <PlaceholderSuggestion className="pb-2 w-60"/>
              <span className="font-semibold">พัฒนาศักยภาพทางวิทยาศาสตร์</span>
            </div>
          </div>
        </div>
      </div>
      </div>
      <Footer/>
    </Layout>
  )

}

export default Page
